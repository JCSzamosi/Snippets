...
# Scale the loadings (I don't fully understand this part)
mult = min(
...
# top 2 for now. This is more useful when there are lots of variables.
load_use = loads[1:2,]
...
p = ggplot(pca_df, aes_string(x = x, y = y)) +
	geom_point(alpha = 0.6, # plot points & colour by grouping
			   aes(colour = Species)) + 
	coord_equal() + # force the axes to have the same scale
	geom_text(data = load_use, # Add the biplot arrow labels:
			  aes(x = v1, y = v2, label = varnames), # required
			  size = 3,
			  vjust = -1,
			  colour = 'grey35') + # play with these freely
	geom_segment(data = load_use, # Add the biplot arrows:
				 aes(x = 0, y = 0, xend = v1, yend = v2), # required
				 arrow = arrow(length = unit(0.2,'cm')),
				 alpha = .75,
				 colour = 'red') + # play with these freely
	labs(x = xlab,y = ylab)
p
```